...
class ClientPermission(BasePermission):
    def has_permission(self, request, view):
        ...
        """
        user_role = request.user.role
        if user_role == 'MANAGEMENT' and request.method == 'GET':
            return True
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if user_role == 'SALES' and request.method in ['GET', 'PUT']:
            if obj in Client.objects.filter(id=obj.id, client_contract__sales_contact=request.user):
                return True
        return False
...
class EventPermission(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if user_role == 'MANAGEMENT' and request.method in ['GET', 'PUT']:
            return True
        if user_role == 'SUPPORT' and obj.support_contact == request.user and request.method in ['GET', 'PUT']:
            return True
        return False
